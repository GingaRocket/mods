...
public sealed class RadarMarkerRenderer(ILog log) : IMarkerRenderer
{
    ...
    /// <summary>
    /// Every blip type, because Complete Edition's reported type cannot be trusted: our own
    /// coordinate blips come back as <see cref="BlipType.Ped"/>. Anything that searches the
    /// map has to look everywhere and identify by sprite instead.
    /// </summary>
    ...
    public void Update(CollectibleKey key, MarkerState state)
    {
        ...
        try
        {
            ...
            {
                ApplyStyle(marker.Blip, state.Style);
    ...
    private static void ApplyStyle(Blip blip, MarkerStyle style)
    {
        blip.Color = (BlipColor)(int)style.Colour;
        blip.Scale = style.Scale;
    ...
    /// <summary>
    /// Whether a sprite name is one this renderer hands out. Reporting only.
    /// </summary>
    /// <remarks>
    /// Note what this cannot tell you: GTA IV uses these same destination sprites for its own
    /// friend and activity blips, so a match here means "looks like ours", never "is ours".
    ...
    private static bool IsMarkerSprite(string icon) =>
        icon is nameof(BlipIcon.Misc_Destination1) or nameof(BlipIcon.Misc_Destination2);

    private static void ApplyIcon(Blip blip, CollectibleCategory category)
    {
        ...
        // drawing an indicator. The two destination variants are small neutral markers whose
        // category is communicated by colour rather than by impersonating a game activity.
        blip.Icon = category switch
        {
            CollectibleCategory.Bird => BlipIcon.Misc_Destination1,
            CollectibleCategory.StuntJump => BlipIcon.Misc_Destination2,
            _ => BlipIcon.Misc_Destination1,
        };
    }
